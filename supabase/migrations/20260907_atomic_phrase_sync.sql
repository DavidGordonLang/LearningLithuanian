-- Apply to a staging Supabase project and verify RLS before the release build.
...
begin
  ...
  delete from public.phrases where user_id = owner_id;
  insert into public.phrases(user_id, data)
    select owner_id, value from jsonb_array_elements(p_rows);
  -- Any insertion/constraint/RLS failure rolls back the deletion in this call.
