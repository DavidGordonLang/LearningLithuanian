...
const STR = {
  en: {
    ...
    labels: {
      english: "English", lithuanian: "Lithuanian", phonetic: "Phonetic", category: "Category", usage: "Usage", notes: "Notes",
      rag: "RAG", sheet: "Sheet", edit: "Edit", delete: "Delete", save: "Save", cancel: "Cancel",
    },
  ...
  lt: {
    ...
    labels: {
      english: "Anglų", lithuanian: "Lietuvių", phonetic: "Tarimas", category: "Kategorija", usage: "Naudojimas", notes: "Pastabos",
      rag: "RAG", sheet: "Lapas", edit: "Redaguoti", delete: "Šalinti", save: "Išsaugoti", cancel: "Atšaukti",
    },
...
const loadData = () => { try { const raw = localStorage.getItem(LS_KEY); if (!raw) return []; const arr = JSON.parse(raw); return Array.isArray(arr) ? arr : []; } catch { return []; } };
const todayKey = () => new Date().toISOString().slice(0,10);
const loadStreak = () => { try { const s = JSON.parse(localStorage.getItem(LSK_STREAK) || "null"); if (!s || typeof s.streak !== "number") return { streak: 0, lastDate: "" }; return s; } catch { return { streak: 0, lastDate: "" }; } };
const saveStreak = (s) => localStorage.setItem(LSK_STREAK, JSON.stringify(s));
const loadXp = () => { try { const v = Number(localStorage.getItem(LSK_XP) || "0"); return Number.isFinite(v) ? v : 0; } catch { return 0; } };
const saveXp = (xp) => localStorage.setItem(LSK_XP, String(xp));

// -------------------- Utils --------------------
function daysBetween(d1,d2){const a=new Date(d1+"T00:00:00");const b=new Date(d2+"T00:00:00");return Math.round((b-a)/86400000);}
function normalizeRag(icon=""){const s=String(icon).trim();const low=s.toLowerCase();if(["🔴","🟥","red"].includes(s)||low==="red")return"🔴";if(["🟠","🟧","🟨","🟡"].includes(s)||["amber","orange","yellow"].includes(low))return"🟠";if(["🟢","🟩","green"].includes(s)||low==="green")return"🟢";return"";}
...
function rowKey(r){return `${(r.Sheet||"").trim()}||${normKey(r.English)}||${normKey(r.Lithuanian)}`;}
function mergeRows(existing,incoming){
  const map=new Map(existing.map(r=>[rowKey(r),r]));
  for(const r of incoming){
    const k=rowKey(r);
    if(!map.has(k)){ map.set(k,{...r,"RAG Icon":normalizeRag(r["RAG Icon"])}); }
    else{
      const cur=map.get(k);
      map.set(k,{...cur,...r,English:cur.English||r.English,Lithuanian:cur.Lithuanian||r.Lithuanian,"RAG Icon":normalizeRag(r["RAG Icon"]||cur["RAG Icon"]), __stats: cur.__stats});
    }
  }
...
function exportJson(rows){
  ...
  const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download="lt-phrasebook.json"; a.click(); URL.revokeObjectURL(url);
}
...
export default function App(){
  ...
  const [starterOpen,setStarterOpen]=useState(()=>{try{const hasData=(loadData()||[]).length>0; const seen=!!localStorage.getItem(LSK_ONBOARDED); return !hasData&&!seen;}catch{return true;}});
  ...
  const levelBadge=(lvl)=>{if(lvl>=1000)return"🔱";if(lvl>=500)return"🧠";if(lvl>=200)return"🚀";if(lvl>=100)return"🌟";if(lvl>=50)return"👑";if(lvl>=20)return"🏆";if(lvl>=10)return"🥇";if(lvl>=5)return"🥈";return"🥉";};
  ...
  async function loadStarter(choice){
    const map={ enlt:"/data/starter_en_to_lt.json", lten:"/data/starter_lt_to_en.json", both:"/data/starter_combined_dedup.json" };
    const path=map[choice];
    const label=choice==="both"?"Starter (Both)":choice==="enlt"?"Starter (EN→LT)":"Starter (LT→EN)";
    try{ const incoming=await fetchStarter(path,label); setRows(prev=>mergeRows(prev,incoming)); localStorage.setItem(LSK_ONBOARDED,"1"); setStarterOpen(false); alert(`${label} merged successfully.`); }
  ...
  function saveEdit(globalIdx){ const clean={...editDraft,"RAG Icon":normalizeRag(editDraft["RAG Icon"])}; setRows(prev=>prev.map((r,i)=>i===globalIdx?clean:r)); setEditIdx(null); }
  function cancelEdit(){ setEditIdx(null); }
  function remove(globalIdx){ if(!confirm("Delete this entry?")) return; setRows(prev=>prev.filter((_,i)=>i!==globalIdx)); }
  async function onImportFile(e){
    const f=e.target.files?.[0]; if(!f) return;
    try{
      const newRows=await importXlsx(f);
      if(!newRows.length){ alert("No rows found in workbook."); return; }
  ...
  function applyRagUpdateForAnswer(row,{correct,sessionId}){
    const rag=normalizeRag(row["RAG Icon"]) || "🟠"; // unknown → treat as amber
    ...
    if(correct){
      if(rag==="🔴"){
        ...
        if(ns.redCorrectSessions >= 5){
          newRag="🟠";
          // reset when entering amber fresh
          ns.redCorrectSessions=0; ns.lastCreditedSessionForRed="";
          ns.amberCorrectSessions=0; ns.lastCreditedSessionForAmber="";
          ns.amberWrongSessions=0; ns.lastWrongCreditedSessionForAmber="";
        }
      }else if(rag==="🟠"){
        ...
        if(ns.amberCorrectSessions >= 5){
          newRag="🟢";
          // leaving amber → clear amber counters
          ns.amberCorrectSessions=0; ns.lastCreditedSessionForAmber="";
          ns.amberWrongSessions=0; ns.lastWrongCreditedSessionForAmber="";
        }
      }
      ...
      if(rag==="🟢"){
        // demote to amber and reset amber counters
        newRag="🟠";
        ns.amberCorrectSessions=0; ns.lastCreditedSessionForAmber="";
        ns.amberWrongSessions=0; ns.lastWrongCreditedSessionForAmber="";
      }else if(rag==="🟠"){
        // count wrong only once per session while amber
        ...
        if(ns.amberWrongSessions >= 3){
          // demote to red and clear amber counters
          newRag="🔴";
          ns.amberWrongSessions=0; ns.lastWrongCreditedSessionForAmber="";
          ns.amberCorrectSessions=0; ns.lastCreditedSessionForAmber="";
        }
  ...
  function startQuiz(){
    if(rows.length<4){ alert("Add more entries first (need at least 4)."); return; }
    const pool=computeQuizPool(rows,10);
    if(!pool.length){ alert("No quiz candidates found."); return; }
  ...
  function quitQuiz(){ if(!confirm("Quit the quiz? Your progress for this session won't count toward streak.")) return; setQuizOn(false); }
  ...
  async function answerQuiz(option){
    ...
    // apply RAG rules to the library row
    const k=rowKey(item);
    setRows(prev=>{
      ...
      <div className="sticky top-0 z-10 bg-zinc-950/80 backdrop-blur border-b border-zinc-800">
        <div className="max-w-xl mx-auto px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between gap-2 sm:gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <select className="bg-zinc-900 border border-zinc-700 rounded-md text-xs px-2 py-1 flex-1 sm:flex-none"
              value={voiceName} onChange={(e)=>setVoiceName(e.target.value)} disabled={ttsProvider!=="browser"}
        ...
        <div className="max-w-xl mx-auto px-3 sm:px-4 pb-2 sm:pb-3 flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            ...
            {["EN2LT","LT2EN"].map(m=>(
              <button key={m} onClick={()=>setDirection(m)}
          ...
          <div className="flex items-center gap-1">
            {["","🔴","🟠","🟢"].map((x,i)=>(
              <button key={i} onClick={()=>setRagPriority(x)}
      ...
      {quizOn && (
        <div className="max-w-xl mx-auto px-3 sm:px-4 pb-28">
          <div className="mt-3 mb-2 flex items-center justify-between">
            <div className="text-sm text-zinc-400">{uiLang==="lt"?"Klausimas":"Question"} {quizIdx+1} / {quizQs.length}</div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3">
              {(()=>{const item=quizQs[quizIdx]; const questionText=item.English; const correctLt=item.Lithuanian;
                ...
                return (<>
                  ...
                  <div className="space-y-2">
                    {quizOptions.map(opt=>{
                      const isSelected=quizChoice===opt; const isCorrect=opt===correctLt; const showColors=quizAnswered;
                      const base="w-full text-left px-3 py-2 rounded-md border flex items-center justify-between gap-2";
                      const color=!showColors?"bg-zinc-900 border-zinc-700":isCorrect?"bg-emerald-700/40 border-emerald-600":isSelected?"bg-red-900/40 border-red-600":"bg-zinc-900 border-zinc-700";
                      return (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="w-[92%] max-w-md bg-zinc-900 border border-zinc-700 rounded-2xl p-4">
            ...
            <div className="space-y-4 text-sm">
              ...
              {/* Azure config (visible when Azure selected) */}
              {ttsProvider==="azure" && (
                <div className="space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <div className="text-xs mb-1">{t("azure.key")}</div>
                      <input type="password" value={azureKey} onChange={(e)=>setAzureKey(e.target.value)} placeholder="Azure key"
                        className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-3 py-2"/>
                    </div>
                    <div>
                      <div className="text-xs mb-1">{t("azure.region")}</div>
                      <input value={azureRegion} onChange={(e)=>setAzureRegion(e.target.value)} placeholder="westeurope"
                        className="w-full bg-zinc-950 border border-zinc-700 rounded-md px-3 py-2"/>
                    </div>
                  </div>
      ...
      {quizShowCongrats && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="w-[92%] max-w-sm bg-zinc-900 border border-zinc-700 rounded-2xl p-5 text-center">
            ...
            <div className="text-zinc-300 mb-1">{uiLang==="lt"?"Jūsų rezultatas":"You scored"} {quizScore} / {quizQs.length}.</div>
            <div className="text-sm text-emerald-400 mb-2">+{quizSessionXp} XP</div>
            {1+Math.floor(xp/XP_PER_LEVEL)>quizStartLevel && (
              <div className="text-sm mb-2">{uiLang==="lt"?"Lygiu aukštyn!":"Level Up!"} {levelBadge(1+Math.floor(xp/XP_PER_LEVEL))} {uiLang==="lt"?"Dabar":"Now"} <span className="font-semibold">{STR[uiLang].level} {numberWithCommas(1+Math.floor(xp/XP_PER_LEVEL))}</span></div>
            )}
