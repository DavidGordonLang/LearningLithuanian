 ...
 * - Pointer-based long-press for slow audio (no duplicate plays)
...
const STARTERS = {
  ...
  NUMBERS: "/data/starter_numbers.json",
};
...
const loadXP = () => Number(localStorage.getItem(LSK_XP) || "0");
...
export default function App() {
  ...
  const levelProgress = xp % LEVEL_STEP;
    ...
    return (
      <div className="max-w-6xl mx-auto px-3 sm:px-4 pb-24">
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <button onClick={() => fetchStarter("NUMBERS")} className="bg-zinc-900 border border-zinc-700 rounded-md px-3 py-2">
            {T.installNums}
          </button>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      {/** Quiz modal inline (simple) */}
      {/** Opened via Start Quiz button in header */}
      {/** Renders over page content */}
      {quizOn && (
